...
export async function fileToCompressedDataUrl(file, { maxDim = 900, quality = 0.75 } = {}) {
  ...
  try {
    // Attempt 1: Modern, memory-efficient way (decodes off main thread)
    ...
    try {
      // Attempt 2: Fallback to Object URL
      img = await new Promise((resolve, reject) => {
        const i = new Image();
        const url = URL.createObjectURL(file);
        i.onload = () => { URL.revokeObjectURL(url); resolve(i); };
        i.onerror = () => { URL.revokeObjectURL(url); reject(new Error('Object URL failed')); };
        i.src = url;
      });
      width = img.width;
      height = img.height;
    } catch (err2) {
      // Attempt 3: Fallback to FileReader (heavy on memory but works on older/weird browsers)
      try {
        img = await new Promise((resolve, reject) => {
          ...
          fr.onload = () => {
            const i = new Image();
            i.onload = () => resolve(i);
            i.onerror = () => reject(new Error('Format foto tidak didukung atau memori penuh.'));
            i.src = fr.result;
          };
          fr.onerror = () => reject(new Error('Gagal membaca file foto.'));
          fr.readAsDataURL(file);
        });
        width = img.width;
        height = img.height;
      } catch (err3) {
        throw new Error("Format tidak didukung atau memori penuh saat memuat foto.");
      }
    }
  }
