import { defaultDimensionsList, defaultStatements } from '../data/constants.js';
...
export const dimensions = defaultDimensionsList;
...
export function computeSaScores(responses) {
  const dimensionScores = {};
  let weightedSum = 0;
  let totalWeight = 0;

  for (const dim of defaultDimensionsList) {
...
export function validateSaResponses(responses) {
  if (!responses || typeof responses !== 'object') return 'Jawaban tidak valid.';
  for (const s of defaultStatements) {
